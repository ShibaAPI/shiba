...
def test_retrieve_obj_from_url(monkeypatch):
    ...
    assert (
        "body" in obj.content.body.tag
        and obj.content.body == "Don't forget me this weekend!"
    )
...
def test_url_constructor_with_accent():
    connection = ShibaConnection("test", "test")
    action = "genericimportreport"
    ret = inf_constructor(connection, action, inf1="é", inf2="à")
    url = url_constructor(connection, ret)
    assert (
        "https://ws.fr.shopping.rakuten.com/stock_ws?action=genericimportreport&inf1=%C3%A9&inf2=%C3%A0&login=test&"
...
def test_url_constructor():
    ...
    url = url_constructor(connection, ret)
    assert (
        "https://ws.fr.shopping.rakuten.com/stock_ws?action=genericimportreport&inf1=info1&inf2=info2&login=test&"
        "pwd=test&version=2017-02-10" == url
    )
...
def test_assert_quota_exeeded(monkeypatch):
    ...
    monkeypatch.setattr(
        "shiba.shibatools.post_request",
        make_simple_text_mock("quota_exceeded_message.xml"),
    )
